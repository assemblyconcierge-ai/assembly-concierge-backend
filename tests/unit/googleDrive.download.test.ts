...
function fileResponse(size = 1024, extraHeaders: Record<string, string> = {}): Response {
  const body = new Uint8Array(size);
  body.set(Buffer.from('%PDF-1.7'));
...
function download(sourceUrl = VALID_SOURCE, jotformApiKey = TEST_API_KEY) {
